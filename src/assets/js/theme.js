  ...
  bsTooltips: () => {
    ...
    var tooltipList = tooltipTriggerList.map(function (tooltipTriggerEl) {
        return new bootstrap.Tooltip(tooltipTriggerEl, {
            ...
            html: true, 
            title: function () {
                var htmlClass = tooltipTriggerEl.getAttribute('data-bs-html-class');
                if (htmlClass) {
                    var contentElement = document.querySelector(`.${htmlClass}`);
                    if (contentElement) {
                        return contentElement.innerHTML; // Передаем HTML как есть
                    }
                }
                return tooltipTriggerEl.getAttribute('title');
            }
        });
    });
},
...
cf7BlockDoubleClick: function () {
  ...
  forms.forEach(function (form) {
    ...
    // При клике на кнопку — меняем текст и блокируем кнопку
    submitButton.addEventListener("click", function () {
      submitButton.disabled = true;
      if (submitButton.tagName === "input") {
        submitButton.value = "Отправка...";
      } else {
        submitButton.innerHTML = `Отправка... <i class="uil uil-envelope-upload ms-2"></i>`;
      }
    });
    ...
    // Обработчики всех возможных событий завершения CF7
    form.addEventListener("wpcf7mailsent", function () {
      resetButton();

      // Модалка после успешной отправки
    ...
    // Если форма не отправилась из-за ошибки или спама — сбрасываем кнопку
    form.addEventListener("wpcf7invalid", resetButton);
    form.addEventListener("wpcf7spam", resetButton);
    form.addEventListener("wpcf7error", resetButton);
  });
},
