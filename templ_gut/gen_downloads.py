...
# -*- coding: utf-8 -*-
"""
Batch Gutenberg generator — outputs to Downloads folder organised by industry.
Reads JSON from Downloads/barber/sites/  (all 86 files)
Outputs TXT to   Downloads/barber/{group}/{filename}.txt
"""
...
JSON_DIR   = r'C:\Users\gigam\Downloads\codeweber сайт\barber\sites'
OUTPUT_BASE = r'C:\Users\gigam\Downloads\codeweber сайт\barber'

# ─────────────────────────────────────────────
# Source strings (barbershop template)
# ─────────────────────────────────────────────
...
def build_replacements(data):
    ...
    # 5. Advantage item 0
    adv_items = c.get('advantages', {}).get('items', [])
    if adv_items:
        adv0 = adv_items[0]
        tgt_adv_t = adv0['title']
        tgt_adv_p = adv0['text']
    else:
        tgt_adv_t = SRC_ADVANTAGE_0_TITLE
        tgt_adv_p = SRC_ADVANTAGE_0_TEXT
    reps.append((
    ...
    # 6. About name
    name = s['name']
    reps.append((f'{SRC_ABOUT_NAME} — это команда', f'{name} — это команда'))
    ...
    # 10. Works eyebrow
    works_eyebrow = c.get('sectionHeads', {}).get('works', {}).get('eyebrow', 'Наши работы')
...
def main():
    ...
    for json_path in json_files:
        ...
        try:
            ...
            # Determine output folder from meta.group
            group = data.get('meta', {}).get('group', 'Прочее')
            out_dir = os.path.join(OUTPUT_BASE, group)
            ...
            issues = verify(result, data)
            if issues:
                warn += 1
                print(f'  WARN  {group}/{basename}.txt — {"; ".join(issues)}')
            else:
                ok += 1
                print(f'  OK    {group}/{basename}.txt')
    ...
    for entry in sorted(os.scandir(OUTPUT_BASE), key=lambda e: e.name):
        if entry.is_dir():
            count = len([f for f in os.listdir(entry.path) if f.endswith('.txt')])
            print(f'  {entry.name}/  ({count} files)')
