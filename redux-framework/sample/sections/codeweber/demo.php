				...
				<div class="demo-controls" style="margin: 20px 0;">
					...
					<div style="margin: 15px 0;">
						<button id="cw-demo-create-events" class="button button-primary" style="margin-right: 10px;">
							' . esc_html__( 'Create Demo Events', 'codeweber' ) . '
						</button>
						<button id="cw-demo-delete-events" class="button button-secondary">
							' . esc_html__( 'Delete Demo Events', 'codeweber' ) . '
						</button>
					</div>
					<div id="cw-demo-events-status" class="demo-status" style="margin-top: 10px; padding: 10px; background: #f0f0f0; border-radius: 4px; display: none;"></div>
				</div>
				<script>
				(function($) {
					"use strict";

					var createNonce = "' . wp_create_nonce( 'cw_demo_create_events' ) . '";
					var deleteNonce = "' . wp_create_nonce( 'cw_demo_delete_events' ) . '";

					function showStatus(message, type) {
						var $status = $("#cw-demo-events-status");
					...
					function setButtonsState(disabled) {
						$("#cw-demo-create-events, #cw-demo-delete-events").prop("disabled", disabled);
					}

					$("#cw-demo-create-events").on("click", function(e) {
						e.preventDefault();

						if (!confirm("' . esc_js( __( 'Create 50 demo events? This may take a while (image import).', 'codeweber' ) ) . '")) {
							return;
						}
					...
					$("#cw-demo-delete-events").on("click", function(e) {
						...
						if (!confirm("' . esc_js( __( 'Delete all demo events? This action cannot be undone.', 'codeweber' ) ) . '")) {
							return;
						}

						setButtonsState(true);
						showStatus("' . esc_js( __( 'Deleting events...', 'codeweber' ) ) . '", "info");

						$.ajax({
							url: ajaxurl,
							type: "POST",
							data: {
								action: "cw_demo_delete_events",
								nonce: deleteNonce
							},
							success: function(response) {
								setButtonsState(false);
