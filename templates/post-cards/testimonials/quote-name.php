<?php
/**
 * Template: Testimonial Quote Name (large centered quote, name only)
 *
 * Borderless extra-large centered quote, author name only (no role, no avatar).
 *
 ...
 * @param array $template_args    Extra args (enable_link, ...)
 */
...
$name_tag   = (!empty($ds['title_tag']) && in_array($ds['title_tag'], $allowed_tags, true)) ? $ds['title_tag'] : 'h6';
$name_class = (isset($ds['title_class']) && $ds['title_class'] !== '') ? $ds['title_class'] : 'mb-0';

ob_start();
?>
<blockquote class="border-0 fs-24 mb-0 text-center">
