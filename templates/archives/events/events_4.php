<?php
/**
 * Archive template: Events — view 4 (horizontal card with image, button)
 *
 * Pattern: card-horizontal — image left (figure with hover-overlay),
 * content right (date, title, details list, "Details" button).
 * Based on vacancies_5 / style4-card.
 *
 * @package Codeweber
 */
				...
				<div class="row <?php echo esc_attr( $grid_gap ); ?> mb-5">
					...
					<div class="col-12">
						<div class="card card-horizontal<?php echo $card_radius ? ' ' . esc_attr( $card_radius ) : ''; ?>">
							<figure class="card-img overlay overlay-1 hover-scale<?php echo $figure_radius ? ' ' . esc_attr( trim( $figure_radius ) ) : ''; ?>">
							...
							<div class="card-body">
								...
								<?php endif; ?>
								<ul class="list-unstyled cc-2 mb-4">
									<?php if ( $location ) : ?>
								...
								<div data-group="page-title-buttons" class="text-end">
									<a href="<?php the_permalink(); ?>" class="btn btn-primary btn-icon btn-icon-start has-ripple<?php echo esc_attr( $btn_style ); ?>">
										<i class="uil uil-arrow-right"></i><?php esc_html_e( 'Details', 'codeweber' ); ?>
									</a>
								</div>
							</div><!-- /.card-body -->
						</div><!-- /.card -->
