...
<div class="product_meta mt-3 pt-3 border-top">
	...
	echo wc_get_product_category_list(
		$product->get_id(),
		', ',
		'<span class="d-block text-muted mb-1 posted_in">' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'woocommerce' ) . ' ',
		'</span>'
	);
	?>
	...
	echo wc_get_product_tag_list(
		$product->get_id(),
		', ',
		'<span class="d-block text-muted mb-1 tagged_as">' . _n( 'Tag:', 'Tags:', count( $product->get_tag_ids() ), 'woocommerce' ) . ' ',
		'</span>'
	);
	?>
