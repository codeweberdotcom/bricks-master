 ...
 *
 * Overridden in theme for styling (btn, getThemeButton, list-unstyled).
 *
		...
		<div id="payment" class="woocommerce-Payment card<?php echo $payment_card_class ? ' ' . esc_attr( $payment_card_class ) : ''; ?> card-body">
			<ul class="woocommerce-PaymentMethods payment_methods methods list-unstyled">
				...
				foreach ( $available_gateways as $gateway ) :
					?>
					<li class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $gateway->id ); ?> payment_method_<?php echo esc_attr( $gateway->id ); ?> mb-3">
						<div class="form-check">
							<input id="payment_method_<?php echo esc_attr( $gateway->id ); ?>" type="radio" class="form-check-input input-radio" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?> />
							<label for="payment_method_<?php echo esc_attr( $gateway->id ); ?>" class="form-check-label"><?php echo wp_kses_post( $gateway->get_title() ); ?> <?php echo wp_kses_post( $gateway->get_icon() ); ?></label>
						</div>
						<?php
						if ( $gateway->has_fields() || $gateway->get_description() ) {
							echo '<div class="woocommerce-PaymentBox woocommerce-PaymentBox--' . esc_attr( $gateway->id ) . ' payment_box payment_method_' . esc_attr( $gateway->id ) . ' mt-2 ps-3" style="display: none;">';
