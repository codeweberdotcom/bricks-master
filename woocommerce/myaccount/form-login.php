	...
	<div class="col login-form-wrapper" style="display: <?php echo $show_register ? 'none' : 'block'; ?>;">
		...
		<form class="woocommerce-form woocommerce-form-login text-center needs-validation" method="post" novalidate>
			...
			<div class="form-floating mb-4">
				<input id="username" type="text" class="form-control<?php echo esc_attr( $form_radius ); ?>" name="username" autocomplete="username" placeholder="<?php esc_attr_e('Username or email address', 'woocommerce'); ?>" value="<?php echo (! empty($_POST['username']) && is_string($_POST['username'])) ? esc_attr(wp_unslash($_POST['username'])) : ''; ?>" required aria-required="true" />
				<label for="username"><?php esc_html_e('Username or email address', 'woocommerce'); ?></label>
			</div>

			<div class="form-floating mb-4 password-field">
				<input id="password" type="password" class="form-control<?php echo esc_attr( $form_radius ); ?>" name="password" autocomplete="current-password" placeholder="<?php esc_attr_e('Password', 'woocommerce'); ?>" required aria-required="true" />
			...
			<p class="form-row">
				...
				<button type="submit" class="woocommerce-button btn btn-primary<?php echo $theme_btn; ?> woocommerce-form-login__submit<?php echo esc_attr(wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : ''); ?>" name="login" value="<?php esc_attr_e('Log in', 'woocommerce'); ?>"><?php esc_html_e('Log in', 'woocommerce'); ?></button>
			</p>
		...
		<div class="col register-form-wrapper" style="display: <?php echo $show_register ? 'block' : 'none'; ?>;">
			...
			<form method="post" class="woocommerce-form woocommerce-form-register needs-validation" <?php do_action('woocommerce_register_form_tag'); ?> novalidate>
				...
				<?php if ('no' === get_option('woocommerce_registration_generate_username')) : ?>
					<div class="form-floating mb-4">
						<input type="text" class="form-control<?php echo esc_attr( $form_radius ); ?>" name="username" id="reg_username" placeholder="<?php esc_attr_e('Username', 'woocommerce'); ?>" value="<?php echo (!empty($_POST['username'])) ? esc_attr(wp_unslash($_POST['username'])) : ''; ?>" required aria-required="true" autocomplete="username" />
						<label for="reg_username"><?php esc_html_e('Username', 'woocommerce'); ?> *</label>
					</div>
				<?php endif; ?>

				<div class="form-floating mb-4">
					<input type="email" class="form-control<?php echo esc_attr( $form_radius ); ?>" name="email" id="reg_email" placeholder="<?php esc_attr_e('Email address', 'woocommerce'); ?>" value="<?php echo (!empty($_POST['email'])) ? esc_attr(wp_unslash($_POST['email'])) : ''; ?>" required aria-required="true" autocomplete="email" />
					<label for="reg_email"><?php esc_html_e('Email address', 'woocommerce'); ?> *</label>
				</div>

				<?php if ('no' === get_option('woocommerce_registration_generate_password')) : ?>
					<div class="form-floating mb-4 password-field">
						<input type="password" class="form-control<?php echo esc_attr( $form_radius ); ?>" name="password" id="reg_password" placeholder="<?php esc_attr_e('Password', 'woocommerce'); ?>" required aria-required="true" autocomplete="new-password" />
						<span class="password-toggle"><i class="uil uil-eye"></i></span>
				...
				<p class="woocommerce-form-row form-row">
					...
					<button type="submit" class="woocommerce-Button woocommerce-button btn btn-primary<?php echo $theme_btn; ?> <?php echo esc_attr(wc_wp_theme_get_element_class_name('button') ? ' ' . wc_wp_theme_get_element_class_name('button') : ''); ?> woocommerce-form-register__submit" name="register" value="<?php esc_attr_e('Register', 'woocommerce'); ?>">
