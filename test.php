...
function redux_handle_custom_font_upload()
{
   ...
   if (empty($font_files)) {
      wp_send_json_error(__('Font files are required', 'codeweber'));
   }

   // Автоматически определяем имя шрифта из первого WOFF файла
   $detected_font_name = '';
   foreach ($font_files['name'] as $index => $name) {
      if ($font_files['error'][$index] === UPLOAD_ERR_OK) {
         ...
         if (in_array($file_ext, ['woff', 'woff2'])) {
            ...
            $detected_font_name = redux_extract_font_name_from_woff($file_tmp);
   ...
   // Используем определенное имя или предоставленное пользователем
   $final_font_name = $detected_font_name ?: $font_name;

   if (empty($final_font_name)) {
      wp_send_json_error(__('Could not detect font name from files. Please provide a font name.', 'codeweber'));
   }

   // Очищаем имя шрифта от специальных символов
   $final_font_name = sanitize_title($final_font_name);
   $final_font_name = preg_replace('/[^a-z0-9_-]/', '', $final_font_name);

   $fonts_dir = get_template_directory() . '/src/assets/fonts/';
   $font_dir = $fonts_dir . $final_font_name . '/';
   ...
   if (empty($css_content) && $has_woff_files) {
      $css_content = redux_generate_font_css($final_font_name, $font_dir);
      file_put_contents($font_dir . $final_font_name . '.css', $css_content);

      // Формируем сообщение о созданных файлах
      $message = __('Font uploaded successfully!', 'codeweber');
      $message .= '<br><strong>' . __('Detected font name:', 'codeweber') . '</strong> ' . $final_font_name;
      $message .= '<br><strong>' . __('Created files:', 'codeweber') . '</strong>';
      $message .= '<br>• ' . $font_dir . $final_font_name . '.css';

      wp_send_json_success($message);
   }

   wp_send_json_success(__('Font uploaded successfully! Detected font name: ', 'codeweber') . $final_font_name);
}

// Улучшенная функция для извлечения имени шрифта из WOFF файла
function redux_extract_font_name_from_woff($file_path)
{
   ...
   // Читаем больше данных для лучшего определения
   $data = fread($handle, 4096);
   fclose($handle);

   // Паттерны для поиска имени шрифта в разных форматах
   $patterns = [
      '/font-family:\s*[\'"]([^\'"]+)[\'"]/i',
      '/FontName.*?[\'"]?([A-Za-z0-9\s\-]+)[\'"]?/i',
      '/name.*?font.*?name.*?[\'"]?([A-Za-z0-9\s\-]+)[\'"]?/i',
      '/Copyright.*?([A-Za-z0-9\s\-]+)[\x00-\x1F]/i',
      '/UniqueID.*?([A-Za-z0-9\s\-]+)[\x00-\x1F]/i'
   ];
   ...
   foreach ($patterns as $pattern) {
      if (preg_match($pattern, $data, $matches)) {
         ...
         $font_name = preg_replace('/[^\w\s\-]/', '', $font_name);
         if (!empty($font_name) && strlen($font_name) > 2) {
            return $font_name;
         }
      }
   }

   // Альтернативный метод: ищем в бинарных данных
   if (preg_match_all('/[A-Za-z][A-Za-z0-9\s\-]{2,50}/', $data, $matches)) {
      foreach ($matches[0] as $potential_name) {
         // Исключаем common words
         $exclude_words = ['woff', 'ttf', 'otf', 'font', 'type', 'true', 'version', 'copyright'];
         $clean_name = trim($potential_name);
         if (
            !in_array(strtolower($clean_name), $exclude_words) &&
...
{
   // Патtern для поиска существующего импорта
...
<div class="redux-custom-fonts-container">
   ...
   <div class="font-upload-form">
      ...
      <input type="text" id="font-name" placeholder="<?php esc_attr_e('Font Family Name (optional - will be detected automatically)', 'codeweber'); ?>" class="regular-text">
...
<script>
   jQuery(document).ready(function($) {
      ...
      $('#upload-font').on('click', function(e) {
         ...
         // Продолжаем загрузку даже если имя не указано - оно определится автоматически
         ...
         formData.append('font_name', fontName); // Может быть пустым

         for (var i = 0; i < fontFiles.length; i++) {
            formData.append('font_files[]', fontFiles[i]);
         }

         $('#upload-progress').html('<div class="spinner is-active"></div> <?php esc_html_e('Uploading and detecting font name...', 'codeweber'); ?>');
