...
functions/events/event-qr.php                       # QR-код для события (метабокс, SVG, только скачать)
...
archive-events.php                                  # Диспетчер: sidebar-обёртка + events_1.php
```
...
### Структура `archive-events.php`

Диспетчер архива. Определяет позицию сайдбара и оборачивает шаблон в Bootstrap row/col:
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="<?= $content_class ?>">
        <?php get_template_part("templates/archives/events/{$templateloop}"); ?>
      </div>
...
**Важно:** `events_1.php` **не содержит** собственного `<section>/<container>` — они предоставляются диспетчером. Сайдбар управляется через Redux: `sidebar_position_archive_events` и `sidebar_breakpoint_events`. Breakpoint `always` показывает сайдбар и на мобильном; `xl` (дефолт) — только от xl.

---

### Файл `templates/archives/events/events_1.php`
...
## QR-код события (admin metabox)

Файл: [functions/events/event-qr.php](functions/events/event-qr.php)

Метабокс «QR Code» в сайдбаре страницы редактирования события. Генерирует SVG QR-код с URL события, без сохранения в БД.

### Как работает

1. Кнопка «Generate QR» → AJAX `cw_event_generate_qr`
2. PHP загружает `functions/lib/phpqrcode/phpqrcode.php`
3. `QRencode::factory(QR_ECLEVEL_H, 10, 4)->encode($url)` → матрица QR (массив строк, `'1'` = тёмный модуль)
4. SVG строится вручную из `<rect>` элементов, margin=4, `shape-rendering="crispEdges"`
5. Возвращается base64-кодированный SVG → `<img src="data:image/svg+xml;base64,...">` + кнопка «Download SVG»

### Безопасность

- Nonce: `cw_event_qr_{post_id}`
- Проверка `current_user_can('edit_post', $post_id)`

### Формат SVG

```xml
<svg xmlns="..." viewBox="0 0 {total} {total}" shape-rendering="crispEdges">
...
Один `<rect>` на тёмный модуль. `viewBox` в единицах модулей — SVG масштабируется без потерь до любого размера.

### Расширение на другие CPT

Паттерн готов к копированию. Достаточно поменять CPT в `add_meta_box()`, nonce-ключ и источник URL. Библиотека phpqrcode уже в теме, дополнительных зависимостей нет.
