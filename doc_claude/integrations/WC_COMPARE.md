...
При каждом **добавлении** товара в сравнение (не при удалении) PHP отправляет событие в Matomo:

```php
// CW_Compare::track_matomo_compare_add($product_id, $product_name)
...
Срабатывает только при успешном `ajax_toggle`, когда `$added === true`.
...
<div id="cw-compare-bar"
     class="cw-compare-bar position-fixed bottom-0 start-0 end-0 w-100 overflow-hidden
            bg-white border-top shadow-lg py-3 px-4 [is-visible | d-none]">
    <!-- inner: content-compare-bar.php -->
</div>
```

- PHP добавляет `is-visible` если есть товары, иначе `d-none`
- `z-index: 1040` — в SCSS (нет Bootstrap-утилиты для z-1040)
...
$table->has_products(): bool                          // count($products) > 0
$table->collect_attributes(): array                   // ['key' => 'Label', ...] union всех товаров
$table->get_product_attributes( WC_Product ): array   // атрибуты одного товара (taxonomy + custom)
$table->get_attribute_value( WC_Product, $key ): string // значение атрибута или '—'
$table->get_image( $product, $size, $extra_class ): string
$table->get_url( $product ): string
$table->get_name( $product ): string
$table->all_same( $key ): bool              // все товары имеют одно значение атрибута → cw-compare-row--same
$table->show_sku(): bool
...
| `showBar()` | `classList.remove('d-none')` + rAF → `classList.add('is-visible')` |
| `hideBar()` | `classList.remove('is-visible')` → `classList.add('d-none')` по `transitionend` |
...
| `showNotice(msg)` | `CWNotify.show(msg, { type: 'warning', event: 'compare' })` или `alert()` если CWNotify недоступен |
...
| `.cw-compare-col--removing` (таблица) | `pe-none` | Колонка `opacity: .4` (SCSS) + `pointer-events: none` (`pe-none` Bootstrap класс, добавляется JS) |
...
| Блок | Описание |
|------|----------|
| `.cw-compare-bar` | `z-index: 1040; transform: translateY(100%)` → `.is-visible` translateY(0); позиционирование (`position-fixed bottom-0 start-0 end-0 w-100 overflow-hidden`) — Bootstrap HTML-классы |
| `.cw-compare-slot` | 40px mobile / 56px sm+ через media query (нет responsive `w-*` вариантов в теме) |
| `.cw-compare-slot-remove` | `width/height: 18px; top/right: -6px; background: rgba(0,0,0,.6); font-size: 10px` — кнопка удаления слота |
...
| `.cw-compare-col--removing` | `opacity: .4; transition: .25s` (pointer-events — `pe-none` через JS) |
