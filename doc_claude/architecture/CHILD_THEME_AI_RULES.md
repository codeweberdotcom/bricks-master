...
WooCommerce ищет шаблоны по приоритету:

1. `my-child-theme/woocommerce/` ← **дочерняя тема**
2. `codeweber/woocommerce/`
3. `woocommerce/templates/` (плагин)

### Как создать переопределение

1. Найти оригинал в `wp-content/plugins/woocommerce/templates/`
2. Создать файл с тем же путём в `woocommerce/` дочерней темы

```
Оригинал:  woocommerce/templates/single-product/price.php
Родитель:  codeweber/woocommerce/single-product/price.php   (если переопределён)
Дочерняя:  my-child/woocommerce/single-product/price.php    ← создать здесь
```

Если родительская тема уже переопределила шаблон — взять за основу её версию, **не оригинал плагина**.

```
Существующие переопределения в codeweber/woocommerce/:
  single-product.php
  single-product/
    price.php
    review.php
