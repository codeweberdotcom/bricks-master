...
<section id="content-wrapper" class="wrapper">
  <div class="container">
      ...
      $template_file = "templates/archives/faq/{$templateloop}.php";
      ...
      <?php if ($has_template) : ?>
          <?php 
          // Используем шаблон из папки templates/archives/faq
          while (have_posts()) : 
            the_post();
            
            if (locate_template($template_file)) {
                get_template_part("templates/archives/faq/{$templateloop}");
            }
          endwhile; 
          ?>
      <?php else : ?>
          <?php
          // Fallback: используем текущую логику с аккордеонами
          ...
          <?php if ($all_faqs->have_posts()) : ?>
              ...
              if (!empty($faq_categories) && !is_wp_error($faq_categories)) :
                  ...
                  foreach ($faq_categories as $category) :
                      ...
                      if ($category_faqs->have_posts()) :
                          $section_class = ($category_index === 1) ? 'wrapper' : 'wrapper pt-3';
              ...
              <?php
                  ...
                  if ($uncategorized_faqs->have_posts()) :
                      $card_radius = Codeweber_Options::style('card-radius');
              ?>
                      <section id="faq-uncategorized" class="wrapper pt-3">
                          <div class="card<?php echo $card_radius ? ' ' . esc_attr($card_radius) : ''; ?>">
                              <div class="card-body p-10">
