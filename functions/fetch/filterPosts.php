...
function _fp_render_cw_websites_1a( $query ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		echo '<div class="col-md-6 col-xl-4">';
		cw_wfs_include_card( get_the_ID(), 'card-1a' );
		echo '</div>';
	}
	echo '</div>';
}
...
function _fp_render_cw_websites_1c( $query ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		echo '<div class="col-md-6 col-xl-4">';
		cw_wfs_include_card( get_the_ID(), 'card-1c' );
		echo '</div>';
	}
	echo '</div>';
}
...
function _fp_render_cw_websites_2a( $query ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		echo '<div class="col-md-6 col-xl-4">';
		cw_wfs_include_card( get_the_ID(), 'card-2a' );
		echo '</div>';
	}
	echo '</div>';
}
...
function _fp_render_cw_websites_3a( $query ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		echo '<div class="col-md-6 col-xl-4">';
		cw_wfs_include_card( get_the_ID(), 'card-3a' );
		echo '</div>';
	}
	echo '</div>';
}
...
function _fp_render_cw_websites_3b( $query ) {
	...
	while ( $query->have_posts() ) {
		...
		cw_wfs_include_card( get_the_ID(), 'card-3b', [], [ 'screen_height' => $screen_h, 'scroll_mode' => true ] );
		echo '</div>';
	}
	echo '</div>';
}
...
function _fp_render_services_grid( $query, $template ) {
	...
	while ( $query->have_posts() ) {
		...
		if ( $short_desc ) {
			...
			echo '<p class="' . esc_attr( $desc_class ) . '">' . wp_kses( $short_desc, [ 'br' => [] ] ) . '</p>';
		}
