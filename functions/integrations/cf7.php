...
<?php
/**
 * Plugin Name: CF7 URL Parameters to Email
 * Description: Добавляет параметры из URL в конец письма CF7 с кнопкой для быстрой вставки
 * Version: 1.3
 * Author: Your Name
 */

// Добавляем скрытое поле с параметрами URL в форму
add_filter('wpcf7_form_hidden_fields', 'cf7_add_url_params_field');
function cf7_add_url_params_field($fields)
{
   if (!empty($_GET)) {
      $fields['_url_params'] = json_encode($_GET);
   }
   return $fields;
}

// Добавляем параметры URL в тело письма
add_filter('wpcf7_mail_components', 'cf7_add_url_params_to_email', 10, 3);
function cf7_add_url_params_to_email($components, $wpcf7, $mail)
{
   ...
   if ($submission) {
      ...
      if (!empty($data['_url_params'])) {
         $url_params = json_decode(stripslashes($data['_url_params']), true);
         ...
         if (!empty($url_params) && is_array($url_params)) {
            ...
            foreach ($url_params as $key => $value) {
               if (!empty($value)) {
                  $params_text .= sprintf("%s: %s\n", sanitize_text_field($key), sanitize_text_field($value));
               }
            }

            // Добавляем параметры в тело письма
            $components['body'] .= $params_text;
         }
      }
   }
...
// Добавляем шорткод для отображения параметров в форме
add_action('wpcf7_init', 'cf7_url_params_shortcode');
function cf7_url_params_shortcode()
{
   wpcf7_add_form_tag('url_param', 'cf7_url_param_shortcode_handler', array('name-attr' => true));
}

function cf7_url_param_shortcode_handler($tag)
{
   $tag = new WPCF7_FormTag($tag);
   $param_name = $tag->get_option('name', '', true);

   if (!empty($_GET[$param_name])) {
      return esc_attr($_GET[$param_name]);
   }

   return '';
}

// Добавляем кнопку в редактор CF7
add_action('wpcf7_admin_footer', 'cf7_add_url_param_button');
function cf7_add_url_param_button()
{
   global $post;

   if (!isset($post) || $post->post_type != 'wpcf7_contact_form') {
      return;
   }
?>
   <script type="text/javascript">
      document.addEventListener('DOMContentLoaded', function() {
         ...
         var tagButtons = document.querySelector('.tag-generator-panel');
         if (tagButtons) {
            var urlParamButton = document.createElement('button');
            urlParamButton.type = 'button';
            urlParamButton.className = 'button button-secondary';
            urlParamButton.innerHTML = 'URL параметр';
            urlParamButton.style.marginLeft = '10px';
            urlParamButton.onclick = function() {
               var paramName = prompt('Введите имя параметра URL (например: utm_source, ref, etc):', 'utm_source');
               if (paramName && paramName.trim() !== '') {
                  // Вставляем шорткод в активное текстовое поле
                  var activeElement = document.activeElement;
                  if (activeElement && (activeElement.tagName === 'TEXTAREA' || activeElement.tagName === 'INPUT')) {
                     var shortcode = '[url_param name="' + paramName.trim() + '"]';
                     ...
                     if (activeElement.selectionStart !== undefined) {
                        ...
                        activeElement.value = text.substring(0, start) + shortcode + text.substring(end);
                        activeElement.selectionStart = activeElement.selectionEnd = start + shortcode.length;
                     } else {
                        activeElement.value += shortcode;
                     }

                     // Триггерим изменение для обновления preview
                     var event = new Event('input', {
                        bubbles: true
                     });
                     activeElement.dispatchEvent(event);
                  } else {
                     alert('Сначала кликните в поле, куда хотите вставить шорткод');
                  }
               }
            };

            // Вставляем кнопку после существующих кнопок
            tagButtons.parentNode.insertBefore(urlParamButton, tagButtons.nextSibling);
         }

         // Альтернативное размещение кнопки
         setTimeout(function() {
            var contactFormEditor = document.querySelector('.contact-form-editor');
            if (contactFormEditor && !document.querySelector('.cf7-url-param-btn')) {
               var simpleButton = document.createElement('button');
               simpleButton.type = 'button';
               simpleButton.className = 'button cf7-url-param-btn';
               simpleButton.innerHTML = '+ URL параметр';
               simpleButton.style.cssText = 'margin: 10px 0; padding: 10px 15px; background: #0073aa; color: white; border: none; border-radius: 4px; cursor: pointer;';
               simpleButton.onclick = function() {
                  var paramName = prompt('Введите имя параметра URL:', 'utm_source');
                  if (paramName && paramName.trim() !== '') {
                     var shortcode = '[url_param name="' + paramName.trim() + '"]';
                     var textarea = document.querySelector('.contact-form-editor-textarea');
                     if (textarea) {
                        textarea.focus();
               ...
               contactFormEditor.insertBefore(simpleButton, contactFormEditor.firstChild);
            }
         }, 1000);
      });
   </script>
...
// Упрощенный генератор тегов
add_action('wpcf7_admin_init', 'cf7_url_params_add_tag_generator', 55);
function cf7_url_params_add_tag_generator()
{
   ...
   wpcf7_add_tag_generator(
      'url_param',
      'URL параметр',
      'cf7_url_params_tag_generator_panel',
      array('nameless' => true)
   );
}

function cf7_url_params_tag_generator_panel($contact_form, $args = '')
{
?>
   <div class="control-box">
      <fieldset>
         <legend>Генерирует тег для отображения параметров из URL</legend>

         <table class="form-table">
            <tbody>
               <tr>
                  <th scope="row">
                     <label for="tag-generator-panel-url-param-name">
                        Имя параметра
                     </label>
                  </th>
                  <td>
                     <input type="text" name="name" class="tg-name oneline"
                        id="tag-generator-panel-url-param-name" />
                     <br />
                     <small>Например: utm_source, ref, campaign</small>
                  </td>
               </tr>
            </tbody>
         </table>
   ...
   <div class="insert-box">
      <input type="text" name="url_param" class="tag code" readonly="readonly" onfocus="this.select()" />

      <div class="submitbox">
         <input type="button" class="button button-primary insert-tag" value="Вставить тег" />
      </div>
      ...
      <p class="description mail-tag">
         <label>
            <input type="checkbox" name="mailtag" class="option" checked="checked" />
            Использовать этот тег в письме
         </label>
      </p>
   </div>
   ...
   <script type="text/javascript">
      jQuery(document).ready(function($) {
         $('#tag-generator-panel-url-param .insert-tag').on('click', function() {
            var name = $('#tag-generator-panel-url-param-name').val().trim();
            if (name) {
               var tag = '[url_param name="' + name + '"]';
               var $textarea = $('.contact-form-editor-textarea:focus');
               if (!$textarea.length) {
                  $textarea = $('.contact-form-editor-textarea').first();
               }
               if ($textarea.length) {
