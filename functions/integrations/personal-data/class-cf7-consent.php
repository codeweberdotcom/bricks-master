...
/**
 * Класс для обработки согласий в Contact Form 7
 */
class CF7_Consent
{
   ...
   {
      // Включаем поддержку шорткодов внутри HTML-кода форм CF7
      add_filter('wpcf7_form_elements', [$this, 'do_shortcode_in_form']);

      // Добавляем панель Legal Consent
      add_filter('wpcf7_editor_panels', [$this, 'add_legal_consent_panel']);

      // Сохраняем мета-данные формы
      add_action('wpcf7_after_save', [$this, 'save_form_metadata']);

      // Шорткоды
      add_shortcode('cf7_legal_consent_link', [$this, 'legal_consent_link_shortcode']);
      add_shortcode('cf7_mailing_consent_link', [$this, 'mailing_consent_link_shortcode']);
      add_shortcode('cf7_privacy_policy', [$this, 'privacy_policy_shortcode']);

      // Автоматическое обновление шорткодов
      add_action('wpcf7_after_save', [$this, 'update_shortcodes_in_form']);

      // Добавляем панель Field Mapping
      add_filter('wpcf7_editor_panels', [$this, 'add_field_mapping_panel']);
      add_action('wpcf7_save_contact_form', [$this, 'save_field_mapping']);
    ...
    */
   public function do_shortcode_in_form($content)
   {
    ...
    */
   private function save_subscriber_to_cpt($email, $phone = '', $user_id = 0)
   {
    ...
    */
   private function save_consents_to_cpt($subscriber_id, $consents_data)
   {
      if (!class_exists('Consent_CPT')) {
         return false;
      }
   ...
   /**
    * Добавляет панель Legal Consent
    */
   public function add_legal_consent_panel($panels)
   {
      $panels['legal_consent'] = [
         'title'    => __('Legal Consent', 'codeweber'),
         'callback' => [$this, 'render_legal_consent_panel'],
      ];
      return $panels;
   }

   /**
    * Отображает панель Legal Consent
    */
   public function render_legal_consent_panel($contact_form)
   {
...
<?php
   ...
   /**
    * Сохраняет мета-данные формы
    */
   public function save_form_metadata($contact_form)
   {
      $form_id = $contact_form->id();
   ...
   /**
    * Шорткод для ссылки на юридическое согласие
    */
   public function legal_consent_link_shortcode($atts)
   {
      ...
      return esc_url(get_permalink($doc_id));
   }

   /**
    * Шорткод для ссылки на согласие на рассылку
    */
   public function mailing_consent_link_shortcode($atts)
   {
      $atts = shortcode_atts(['id' => 0], $atts);
      $form_id = intval($atts['id']);
   ...
   /**
    * Обновляет шорткоды в форме
    */
   public function update_shortcodes_in_form($contact_form)
   {
      $form_id = $contact_form->id();
      $form_content = $contact_form->prop('form');
   ...
   /**
    * Добавляет панель Field Mapping
    */
   public function add_field_mapping_panel($panels)
   {
      $panels['field_mapping'] = [
         'title'    => __('Field Mapping', 'codeweber'),
         'callback' => [$this, 'render_field_mapping_panel'],
      ];
      return $panels;
   }

   /**
    * Отображает панель Field Mapping
    */
   public function render_field_mapping_panel($contact_form)
   {
      ...
      foreach ($fields as $key => $label) {
         ...
         foreach ($form_fields as $field_name) {
            $selected = selected($this->get_saved_field_mapping($form_id, $key), $field_name, false);
   ...
   /**
    * Получает сохранённое сопоставление поля
    */
   private function get_saved_field_mapping($form_id, $key)
   {
   ...
   public function handle_form_submission($contact_form)
   {
      ...
      $subscriber_id = $this->save_subscriber_to_cpt($email, $phone, 0);
      if (!$subscriber_id) {
         return;
      }
      ...
      if (!empty($data[$privacy_field]) && $privacy_page_id && get_post_status($privacy_page_id) === 'publish') {
         $checkboxtext = $this->get_acceptance_label_html($privacy_field, $form_id);

         $consents_to_save[] = [
            'type'        => 'privacy_policy',
            'document_title' => get_the_title($privacy_page_id),
            'document_url' => get_permalink($privacy_page_id),
            'revision'    => $this->get_latest_revision_link($privacy_page_id),
            'ip'          => $ip_address,
      ...
      if (!empty($data[$mailing_field]) && $mailing_doc_id && get_post_status($mailing_doc_id) === 'publish') {
         $checkboxtext = $this->get_acceptance_label_html($mailing_field, $form_id);

         $consents_to_save[] = [
            'type'        => 'mailing_consent',
            'document_title' => get_the_title($mailing_doc_id),
            'document_url' => get_permalink($mailing_doc_id),
            'revision'    => $this->get_latest_revision_link($mailing_doc_id),
            'ip'          => $ip_address,
            'user_agent'  => $user_agent,
            'date'        => $timestamp,
            'page_url'    => $page_url,
      ...
      if (!empty($data[$processing_field]) && $processing_doc_id && get_post_status($processing_doc_id) === 'publish') {
         $checkboxtext = $this->get_acceptance_label_html($processing_field, $form_id);

         $consents_to_save[] = [
            'type'        => 'pdn_processing',
            'document_title' => get_the_title($processing_doc_id),
            'document_url' => get_permalink($processing_doc_id),
            'revision'    => $this->get_latest_revision_link($processing_doc_id),
            'ip'          => $ip_address,
            'user_agent'  => $user_agent,
            'date'        => $timestamp,
            'page_url'    => $page_url,
      ...
      if (!empty($consents_to_save)) {
         $this->save_consents_to_cpt($subscriber_id, $consents_to_save);
      }
   }

   /**
    * Извлекает HTML-текст согласия
    */
   private function get_acceptance_label_html($acceptance_name, $form_id)
   {
    ...
    */
   private function get_latest_revision_link($post_id)
   {
      if (!$post_id) {
         return '';
      }
